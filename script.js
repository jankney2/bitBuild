...
for (let i = 0; i < buttons.length; i++) {
  if (buttons[i].id !== "video") {
    buttons[i].addEventListener("click", () => {
      document
        .getElementsByClassName("getStarted")[0]
        .classList.toggle("hidden");
    });
  }else {
      buttons[i].addEventListener('click', ()=>{
          document.getElementById('videoDisp').classList.toggle('hidden')
...
document.getElementById('vidExit').addEventListener('click',()=>{
    document.getElementById('videoDisp').classList.toggle('hidden')
    document.getElementById('video').classList.toggle('hidden')
})
